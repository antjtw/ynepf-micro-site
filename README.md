...
│   ├── js/
│   │   ├── includes.js     Dev-only: loads header/footer via fetch
...
The header and footer live once in `assets/partials/`. Each page contains
...
`build.js` stamps the partials into those placeholders and writes fully static
pages to `dist/`. This means the published pages have no client-side fetch, no
flash of unstyled content, and work even when opened directly from disk.

To rebuild after editing any page or partial:

```bash
node build.js
```

During development you can also open the source pages through a local server
without building — `includes.js` will fetch the partials client-side. (This
needs a server; `fetch` of local files is blocked on `file://`.)

## Running locally

The pages use root-relative asset paths (`/assets/...`), so serve from the site
